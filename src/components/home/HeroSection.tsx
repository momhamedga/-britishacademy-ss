...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    }, 10000); // سرعة 10 ثوانٍ للقراءة المريحة
  ...
  return (
    <section
      ...
      className="relative h-screen min-h-[850px] w-full flex items-center overflow-hidden"
    >
      ...
      {/* 3. Mobile Navigation Arrows (ظهرت فقط في الموبايل md:hidden) */}
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 z-30 flex justify-between px-4 pointer-events-none md:hidden">
        <button 
          onClick={prevSlide}
          className="pointer-events-auto p-3 rounded-full bg-white/5 backdrop-blur-md border border-white/10 text-white/50 active:text-[#D4AF37] transition-colors"
        >
          <ChevronLeft size={24} />
        </button>
        <button 
      ...
      {/* 4. Side Navigation (Desktop & Mobile) */}
      <div className="absolute bottom-16 right-8 md:right-16 z-30 flex items-center gap-6 md:gap-12 scale-75 md:scale-100">
        {HERO_SLIDES.map((_, i) => (
          <button
            key={i}
          ...
          >
            ...
            <div className={`h-[1px] transition-all duration-1000 ${current === i ? "w-12 md:w-20 bg-[#D4AF37] shadow-[0_0_10px_rgba(212,175,55,0.5)]" : "w-6 md:w-8 bg-white/5 group-hover:bg-white/20"}`} />
          </button>
        ))}
      </div>
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-3 opacity-20 hidden md:flex">
...
// Animation Variants (تعريف الفيربانتس كما هي)
const containerVariants = { animate: { transition: { staggerChildren: 0.15 } } };
const titleReveal = {
