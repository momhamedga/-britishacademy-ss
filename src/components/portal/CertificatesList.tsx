...
export default function CertificatesList() {
  ...
  if (certs.length === 0) {
    return (
      <div className="max-w-md mx-auto text-center p-10 rounded-4xl bg-white/2 border border-white/5 space-y-6">
...
<div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
  {certs.map((cert, index) => (
    <motion.div
      ...
      className="p-8 bg-navy border border-white/3 rounded-[2.5rem] flex flex-col justify-between gap-8 hover:border-gold/30 transition-all duration-500 group relative overflow-hidden shadow-[0_20px_50px_rgba(2,6,23,0.4)] hover:shadow-[0_30px_60px_rgba(212,175,55,0.05)]"
    >
      {/* 🌌 تأثير الإضاءة الخلفية الفخم عند الـ Hover */}
      <div className="absolute -top-16 -right-16 w-40 h-40 bg-gold/0 group-hover:bg-gold/3 blur-[60px] rounded-full pointer-events-none transition-all duration-700" />
      <div className="absolute -bottom-16 -left-16 w-40 h-40 bg-white/0 group-hover:bg-white/1 blur-[60px] rounded-full pointer-events-none transition-all duration-700" />

      {/* 🛡️ خط جانبي مضيء يعطي إحساس بالـ Premium Identity */}
      <div className="absolute top-1/4 left-0 w-0.5 h-1/2 bg-linear-to-b from-transparent via-gold/0 group-hover:via-gold/50 to-transparent transition-all duration-500" />

      <div className="space-y-5 relative z-10">
        <div className="flex items-center justify-between">
          {/* Badge التخصص بتصميم زجاجي حاد وعسكري */}
          <span className="text-[9px] font-black bg-linear-to-r from-gold to-yellow-600 text-navy px-4 py-1.5 rounded-xl uppercase tracking-[0.2em] italic shadow-[0_4px_15px_rgba(212,175,55,0.2)]">
            {cert.category || "Security"}
          </span>
          
          {/* كود التحقق داخل إطار مخصص يعطي انطباع السرية */}
          <div className="bg-white/2 border border-white/5 px-3 py-1.5 rounded-lg backdrop-blur-md">
        ...
        <h3 className="text-2xl md:text-3xl font-black uppercase text-white tracking-tighter italic leading-none pt-2 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-linear-to-r group-hover:from-white group-hover:to-gold/80 transition-all duration-500">
          {cert.title}
        </h3>
      </div>
      ...
      <div className="flex items-center justify-between pt-5 border-t border-white/5 gap-4 relative z-10">
        ...
        {cert.certificate_url ? (
          <a
            ...
            className="inline-flex items-center gap-2.5 px-6 py-3.5 bg-white/2 hover:bg-linear-to-r hover:from-gold hover:to-yellow-500 border border-white/10 hover:border-transparent text-white hover:text-navy rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 shadow-lg active:scale-95 group/btn"
          >
            <DownloadCloud size={14} className="group-hover/btn:scale-110 transition-transform" /> 
            <span>Download Asset</span>
          </a>
        ) : (
          <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest border border-dashed border-white/5 px-4 py-2 rounded-xl bg-white/1">
