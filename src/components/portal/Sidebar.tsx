...
export default function Sidebar({ studentData }: any) {
  ...
  // تحسين الأداء عبر Memoization للقائمة - تم إضافة زرار "الرئيسية"
  ...
  return (
    <div className="flex flex-col h-full py-10 px-6 text-white select-none">
      ...
      {/* 🛡️ Branding - Tactical Gold Glow UI (مستوحى من الصورة 2) */}
      <div className="mb-14 px-2 relative group cursor-default">
        <div className="flex items-center gap-3.5 mb-5 relative z-10">
          <div className="relative size-10 flex items-center justify-center">
            {/* 🌌 الـ Glow الملكي خلف اللوجو (نفس تأثير الصورة) */}
            <motion.div 
      ...
      {/* 🧭 Navigation - Framer Motion Optimized with Side Indicator */}
      <nav className="flex-1 space-y-2 relative">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              {/* ✨ الـ Active Indicator Glow Line (الأفقي الرفيع زي الصورة) */}
              {isActive && (
                <motion.div 
              ...
              <motion.div
                ...
                className={`flex items-center justify-between px-5 py-4 rounded-2xl transition-all duration-300 relative overflow-hidden ${
                  isActive 
                  ? 'bg-gradient-to-r from-white/[0.08] to-transparent text-white shadow-[0_10px_30px_rgba(0,0,0,0.2)]' 
      ...
      {/* 👤 Footer - Identity & Tactical Exit */}
      <div className="mt-auto pt-8 space-y-5">
        
        {/* Profile Card - Glassmorphism Level 2 (زي تصميم الموبايل) */}
        ...
        {/* 🚨 Terminate Session - Tactical Red Aggressive Force (نفس تصميم الصورة 1) */}
        <motion.button 
          ...
          className={`w-full flex items-center justify-center gap-3.5 py-5 rounded-[1.8rem] transition-all duration-500 font-bold group border-[1.5px] ${
            ...
            : 'bg-[#121A2D] border-red-600 text-red-600 hover:bg-red-600 hover:border-red-600 hover:text-white shadow-[0_15px_40px_rgba(239,68,68,0.15)] hover:shadow-red-600/40'
          }`}
        >
