...
import { BookOpen, Award, Settings, LogOut, ShieldAlert, Zap, Terminal, ShieldCheck, User } from 'lucide-react'
...
interface ProfileDropdownProps {
  ...
  isAdmin?: boolean; // 🎯 حقن بروب فحص صلاحيات الآدمن
  close: () => void;
}

const ProfileDropdown = ({ user, isAdmin = false, close }: ProfileDropdownProps) => {
  return (
    <motion.div 
    ...
    >
      ...
      {isAdmin ? (
        <>
          ...
          <div className="flex items-center gap-4 pb-5 mb-5 border-b border-white/5">
            ...
            <div className="flex flex-col overflow-hidden text-left">
              <span className="text-white text-[11px] font-black uppercase tracking-tight">SIA_COMMANDER</span>
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                <span className="text-emerald-400 text-[8px] uppercase tracking-[0.2em] font-bold">ROOT ADMIN</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            {[
              { icon: Terminal, label: 'Course Deployment Hub', href: '/admin/courses' },
              { icon: User, label: 'Personnel Records (الطلاب)', href: '/admin/users' },
            ...
            <button 
              onClick={async () => {
                close();
                await logout(); // تأكد أن دالة اللوج أوت بتمسح كوكيز الأدمن واليوزر معاً
              }}
        ...
        /* ✅ 2. حالة الـ Student (تظهر فقط لو الطالب مسجل والآدمن مش مسجل) */
        <>
        ...
        /* 🛑 3. حالة الـ Guest (لو مفيش يوزر ولا آدمن مسجلين) */
